...
function initCarouselEngine() {
  ...
  if (counter)
    counter.innerText = `Project ${currentCardIndex + 1} of ${totalCards}`;
}
...
function syncCalcDisplay() {
  ...
  document.getElementById("calcTotalUsd").textContent =
  ...
  document.querySelectorAll(".calc-chip").forEach((chip) => {
    ...
    const matches =
      (cardPledgeQty === 1 && text.includes("1")) ||
...
// Example inside home.js sponsorship calculator trigger:
function launchSponsorPledge() {
  const currentQty = window.sponsorCount || 1;
  const unitPrice = 75;
  const totalAmount = currentQty * unitPrice;

  // Guaranteed execution via the global window bridge
  if (typeof window.openPledgeModal === "function") {
    window.openPledgeModal(totalAmount, currentQty);
  } else {
    console.error("openPledgeModal is not mounted on window.");
  }
}
